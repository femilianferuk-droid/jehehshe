...
ИСПРАВЛЕННАЯ ВЕРСИЯ - все состояния работают
"""
...
async def start_auto_response_listener(user_id: int, account_id: int):
    """Запустить слушатель автоответов для аккаунта"""
    ...
    try:
        # Получаем все автоответы для этого аккаунта
        async with aiosqlite.connect(DATABASE_PATH) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute('''
        ...
        async for message in client.get_chat_history(limit=1):
            # Проверяем каждое новое сообщение
            for response in responses:
                if response['trigger_text'].lower() in message.text.lower():
                    # Отправляем ответ
                    ...
                    logger.info(f"Auto response sent for trigger: {response['trigger_text']}")
                    break
            
            await asyncio.sleep(1)
            
    except Exception as e:
        logger.error(f"Error in auto response listener: {e}")
    finally:
        await client.stop()
...
async def auth_get_phone(message: types.Message, state: FSMContext):
    ...
    account_name = data.get('account_name', 'Аккаунт')
    user_id = message.from_user.id
...
async def auth_get_code(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Получаем название аккаунта из временной таблицы
        async with aiosqlite.connect(DATABASE_PATH) as db:
            cursor = await db.execute(
...
async def broadcast_get_message(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "⏱ **Шаг 2: Настройка задержки**\n\n"
        "Введите задержку между сообщениями в секундах (рекомендуется 2-5 секунд):\n"
        "Например: 2",
        parse_mode="Markdown"
    )

@dp.message(BroadcastStates.waiting_delay)
async def broadcast_get_delay(message: types.Message, state: FSMContext):
    """Получение задержки для рассылки"""
    try:
        ...
        delay = 2  # Задержка по умолчанию
    
    await state.update_data(broadcast_delay=delay)
    ...
    try:
        ...
        await status_msg.edit_text(
            f"📨 **Шаг 3: Выберите чаты для рассылки**\n\n"
            f"Всего доступно чатов: {len(dialogs)}\n"
            f"Можно выбрать до 10 чатов.\n"
            f"Задержка между сообщениями: {delay} сек.\n\n"
...
async def start_broadcast_confirm(callback: types.CallbackQuery, state: FSMContext):
    ...
    delay = data.get('broadcast_delay', 2)
    ...
    preview += f"**Задержка:** {delay} сек.\n"
    preview += f"**Примерное время:** {len(selected) * delay} сек."
...
async def execute_broadcast(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute('''
            ...
            (user_id, account_id, message_text, selected_chats, delay_seconds, status, sent_count, total_count)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            user_id, account_id, message_text[:500], 
            json.dumps(selected), delay, 'completed', sent, len(selected)
        ))
        await db.commit()
    ...
    result_text += f"📊 Всего чатов: {len(selected)}\n"
    result_text += f"⏱ Задержка: {delay} сек."
    
    if failed_chats:
        result_text += f"\n\n❌ Неудачные чаты: {', '.join(failed_chats[:5])}"
        if len(failed_chats) > 5:
            result_text += f" и еще {len(failed_chats) - 5}"
...
async def response_get_text(message: types.Message, state: FSMContext):
    ...
    # Запускаем слушатель для этого аккаунта
    asyncio.create_task(start_auto_response_listener(user_id, account_id))
    
    await state.clear()

@dp.callback_query(F.data.startswith("edit_response_"))
async def edit_response(callback: types.CallbackQuery):
    """Редактирование автоответа"""
    response_id = int(callback.data.replace("edit_response_", ""))
    
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
            "SELECT * FROM auto_responses WHERE id = ?",
            (response_id,)
        )
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="✅ Вкл/Выкл", callback_data=f"toggle_response_{response_id}"),
        InlineKeyboardButton(text="❌ Удалить", callback_data=f"delete_response_{response_id}")
    )
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_main"))
    
    await callback.message.edit_text(
        f"🤖 **Автоответ**\n\n"
        f"**Триггер:** {response['trigger_text']}\n"
        f"**Ответ:** {response['response_text'][:200]}\n"
        f"**Статус:** {'✅ Активен' if response['is_active'] else '❌ Неактивен'}",
        reply_markup=builder.as_markup()
    )
    await callback.answer()

@dp.callback_query(F.data.startswith("toggle_response_"))
async def toggle_response(callback: types.CallbackQuery):
    """Включение/выключение автоответа"""
    response_id = int(callback.data.replace("toggle_response_", ""))
    ...
    await edit_response(callback)

@dp.callback_query(F.data.startswith("delete_response_"))
async def delete_response(callback: types.CallbackQuery):
    """Удаление автоответа"""
    response_id = int(callback.data.replace("delete_response_", ""))
    ...
    await callback.answer("❌ Автоответ удален")
    await callback.message.edit_text("✅ Автоответ удален")
...
async def cmd_profile(message: types.Message):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        cursor = await db.execute(
            "SELECT COUNT(*) FROM auto_responses WHERE user_id = ?",
            (user_id,)
        )
...
async def cmd_stats(message: types.Message):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        cursor = await db.execute("SELECT COUNT(*) FROM auto_responses")
        total_responses = (await cursor.fetchone())[0]
...
async def cmd_users(message: types.Message):
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        cursor = await db.execute('''
            ...
            LEFT JOIN auto_responses r ON u.user_id = r.user_id
            GROUP BY u.user_id
    ...
    for row in rows:
        ...
        text += f"`{user_id}` | @{username} | 📱{accounts} | 🤖{responses} | {status}\n"
...
async def auto_response_listener():
    """Слушатель для автоответов"""
    while True:
        try:
            # Получаем все активные автоответы
            async with aiosqlite.connect(DATABASE_PATH) as db:
                db.row_factory = aiosqlite.Row
                cursor = await db.execute('''
            ...
            # Запускаем слушатель для каждого аккаунта
            for row in active:
                asyncio.create_task(
                    start_auto_response_listener(row['user_id'], row['account_id'])
                )
...
async def on_startup():
    ...
    asyncio.create_task(auto_response_listener())
