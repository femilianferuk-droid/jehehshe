...
async def process_get_code(callback: CallbackQuery):
    ...
    try:
        ...
        try:
            ...
            # Берем САМЫЙ ПЕРВЫЙ чат в списке диалогов
            first_chat = dialogs[0].chat
            logger.info(f"Selected FIRST chat: {first_chat.id} - {first_chat.title or first_chat.first_name or 'No title'}")
            
            # Получаем последние сообщения из первого чата
            messages = []
            ...
            if messages:
                ...
                for msg in messages:
                    if msg.text:
                        ...
                        # Ищем код после слова "код" или "code"
                        ...
                        # Ищем код после слова "пароль" или "password"
                        code_match = re.search(r'[ПпPp][аAaA][рRr][оОoO][лLlL][ьYy][:\s]*(\d{5})', msg.text)
                        if code_match:
                            found_code = code_match.group(1)
                            logger.info(f"Found code after password keyword: {found_code}")
                            break
...
⚠️ <b>Важно:</b> Код действителен в течение нескольких минут!
                    ...
                    # Показываем последние несколько сообщений для отладки
                    debug_text = "Последние сообщения:\n"
                    for i, msg in enumerate(messages[:3]):
                        if msg.text:
                            debug_text += f"{i+1}. {msg.text[:50]}...\n"
                    
                    await loading_msg.edit_text(
                        "❌ <b>Не удалось найти код подтверждения</b>\n\n"
                        "В последних сообщениях первого чата нет 5-значного кода.\n\n"
                        f"{debug_text}\n"
                ...
                await loading_msg.edit_text(
                    "❌ <b>Нет сообщений в первом чате</b>\n\n"
                    "В первом чате отсутствуют сообщения.",
                    reply_markup=get_back_to_menu_keyboard(),
                    parse_mode="HTML"
                )
